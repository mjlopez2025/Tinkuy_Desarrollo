...
try {
    ...
    $query = "
        ...
        FROM $tabla 
        WHERE $campo ~* '^(Adjunto,|Titular,|JTP,|Asociado,|\\.-|\\s-|^\\s*\\.,)'
    ...
    foreach ($registros as $index => $registro) {
        ...
        $nuevo_texto = preg_replace([
            '/^(Adjunto,|Titular,|JTP,|Asociado,|\.-|\s-|\s*,\s*)/i',
            '/^[\s\.\-]+/',
            '/\s*,\s*(?=DNI|DNT|CI|LC)/i'
        ], '', $texto_original);
        ...
        if (($index + 1) % 100 === 0 || ($index + 1) === $total) {
            ...
            $tiempo = time() - $inicio;
            echo "\r🔄 Progreso: " . ($index + 1) . "/$total ($porcentaje%) | ⏱️ $tiempo seg | ❌ Tipos inválidos: $tipos_invalidos";
            flush();
        }
    }
